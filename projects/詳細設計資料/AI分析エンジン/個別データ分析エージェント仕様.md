...
- **インタラクティブ可視化**: Plotlyでグラフを生成し、レポートに埋め込み。
...
6. **レポート生成**: レポートエージェントが結果をPlotlyグラフや表とともにマークダウンレポートにまとめる。
...
### 3.2 出力
- **レポート（Markdown）**: 解析結果をまとめた `.md` ファイル。
  - **内容**:
    ...
    - 静的画像（jpg/png）による可視化（時系列、区間ハイライト、相関図）。
    - 初心者向けサマリ。
  - **例**:
    ```
    # 異常解析レポート
...
- **解析エージェント**:
  - **役割**: 自動EDA、仮説生成（RAG）、動的解析（Pandas AI）、結果検証、ループバック。
